...
<template>
  <div>
    <div id="clear-account-print">
      <Table ref="tableClearAccountPrint" :columns="options.columns" :data="seqDataCompute" size="small"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    printClick() {
      this.$nextTick(() => {
        // printJS({
        //   printable: 'clear-account-print',
        //   type: 'html',
        //   style: legaTablePrintCss,
        //   scanStyles: false
        // })
        domtoimage.toPng(this.$el.querySelector('#clear-account-print')).then(blob => {
          printJS({
            printable: [blob],
            type: 'image'
          })
    ...
    exportData() {
      ...
      this.$nextTick(() => {
        this.$refs.tableClearAccountPrint.exportCsv({
          filename: `${hospitalName}清算流水清单-${this._moment().format('YYYY-MM-DD')}`
        })
      })
    },
    findNoClearSeqExpense(params = {}) {
      if (this._.isEmpty(params.conditions)) {
        params.conditions = []
      }
      params.conditions.push({ field: 'clearStatus', value: '0' }, { field: 'area', value: this.area })
